...
async function ContentPage({ params: { slug, contentType } }: PageProps) {
  const content: any = await fetch(
    `${hostname}/api/getcontent/get-single-content?slug=${slug}&contentType=${contentType}`
  ).then((res) => res.json());
  const ratings: any = await fetch(
    `${hostname}/api/addrating/get?contentid=${content._id}&slug=${slug}&contentType=${contentType}`
  ).then((res) => res.json());
      ...
      <div className="max-w-7xl lg:mx-4 md:mx-4 sm:mx-4 mx-4 xl:mx-auto my-5">
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <h3>
              {content.releaseDate || content.AirDate} • {content.duration} •{" "}
              {content.platform ? content.platform : "In Theatres"}
            </h3>
          </div>
...
export async function generateStaticParams() {
  const contents = await fetch(`${hostname}/api/getcontent/get-slugs`, {
    next: { revalidate: 60 },
  }).then((res) => res.json());
